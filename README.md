...
The skill ships **no code** — it calls the service over HTTP. Install the skill (markdown
only), [start the service](#start-the-service), then set `WATERMARKS_SERVICE_URL` if it is
not `http://127.0.0.1:8765`.

It follows the [Agent Skills](https://agentskills.io) spec (`name`, `description`,
`license`, `compatibility`, `metadata` frontmatter only), so the same directory loads
unchanged in Claude Code, Claude Cowork, claude.ai and any other Agent Skills runtime.
...
### Reaching the service from each host

Since the skill is a thin HTTP client, what differs per host is only which service URL it
can reach. What the service itself can do is reported by `GET /capabilities`, not by the
host.

| Host | Service URL to use |
| --- | --- |
| Claude Code (local) | `http://127.0.0.1:8765` after `make serve` or `docker compose up -d` |
| Cowork, claude.ai, cloud sessions | A routable URL you set in `WATERMARKS_SERVICE_URL` — loopback there is the sandbox, not your machine, so a service on your laptop needs to be exposed (tunnel, LAN address, or a hosted deployment) |

If the sandbox has no outbound access to your service, the skill says so instead of falling
back to hand-cleaning. Protect any exposed deployment with `WATERMARKS_SERVER_API_KEY`.
